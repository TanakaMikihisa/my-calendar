...
protocol TagRepositoryProtocol: Sendable {
    func listActive() async throws -> [Tag]
    func add(tag: Tag) async throws
    func update(tag: Tag) async throws
    func deactivate(tagId: TagID) async throws
}
...
actor FirestoreTagRepository: TagRepositoryProtocol {
    ...
    func listActive() async throws -> [Tag] {
        let snapshot = try await db
            .collection(FirestorePaths.tags)
    ...
    func add(tag: Tag) async throws {
        let ref = await db.collection(FirestorePaths.tags).document(tag.id)
    ...
    func deactivate(tagId: TagID) async throws {
        let ref = await db.collection(FirestorePaths.tags).document(tagId)
    ...
    private func mapTag(doc: QueryDocumentSnapshot) throws -> Tag {
        ...
        return try Tag(
            id: doc.documentID,
            name: FirestoreMappers.string(data["name"], key: "name"),
